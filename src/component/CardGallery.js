...
const chapters = [
  ...
  { name: "Karma Sanyasa Yoga", verses: 29, img: "5", description: "Path of Renunciation" },
  { name: "Dhyana Yoga", verses: 47, img: "6", description: "Path of Meditation" },
  { name: "Gyaan Vigyana Yoga", verses: 30, img: "7", description: "Self - knowledge and Enlightenment" },
  ...
  { name: "Vibhooti Yoga", verses: 42, img: "10", description: "Yoga through Appreciating the Infinite Opulences of God" },
...
const Card = ({ chapter, index, onClick }) => (
  <div 
    className="pt-4 max-w-sm bg-white rounded-lg overflow-hidden shadow-xl transform transition duration-300 hover:scale-105 hover:shadow-2xl" 
    onClick={onClick}
  >
...
function CardGallery() {
  ...
  const handleImageClick = async (img) => {
    try {
      const response = await fetch(`https://api.thegita.org.in/api/v1/sloka/en/${img}`);
      const data = await response.json();
      // Use navigate instead of history.push
      navigate(`/sloka/${img}`, { state: { slokaData: data } }); // Change here
    } catch (error) {
      ...
      {isMobile ? (
            ...
            >
              {chapters.slice(startIndex, startIndex + 3).map((chapter, index) => (
                <SwiperSlide key={index + startIndex}>
                  <Card chapter={chapter} index={index + startIndex} onClick={() => handleImageClick(chapter.img)} />
                </SwiperSlide>
              ))}
            </Swiper>
          ))}
        ...
        <div className="grid md:grid-cols-3 gap-4 pt-4">
          {chapters.map((chapter, index) => (
            <Card key={index} chapter={chapter} index={index} onClick={() => handleImageClick(chapter.img)} />
          ))}
        </div>
      )}
    </div>
